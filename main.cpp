    ...
    if (r >= 0.95) {
        ...
        if (r < 0.45) {
            if (r < 0.1125) {
                ...
                p.color.a = random_alpha();
            }
        } else if (r < 0.9) {
            if (r < 0.675) {
                move_polygon(p);
            } else {
                cv::Point& pt = p.points[random_int(0, SZ(p.points) - 1)];
                if (r < 0.7875) {
            ...
            if (r < 0.925) {
                ...
                if (npts > 3) {
                    p.points.erase(p.points.begin() + random_int(0, npts - 1));
                }
            } else {
                int npts = SZ(p.points);
                if (npts < max_points_per_polygon) {
                    cv::Point pt(random_int(0, img_width), random_int(0, img_height));
                    p.points.insert(p.points.begin() + random_int(0, npts - 1), pt);
                }
            }
        }
    }
...
cv::Mat render(const Drawing& d, uchar* canvas) {
    std::memset(canvas, 0, CANVAS_SZ);
    cv::Mat image(img_height, img_width, CV_8UC(img_channels), canvas);
...
double compute_fitness(const uchar* img1, const uchar* img2) {
    double e = 0;
    int n = CANVAS_SZ;
    for (int i = 0; i < n; i++) {
        e += std::abs(img1[i] - img2[i]);
    }
    return 1 - (e / (255.0 * n));
}
...
cv::Mat evolve(const uchar* orig_img, uchar* canvas) {
    ...
    std::sort(g.drawings.begin(), g.drawings.end(), fitness_cmp_desc);
    return render(g.drawings.front(), canvas);
}
...
int main(int argc, char** argv) {
    ...
    std::unique_ptr<uchar[]> canvas = std::make_unique<uchar[]>(CANVAS_SZ);
    cv::Mat Result = evolve(orig_image.data, canvas.get());
